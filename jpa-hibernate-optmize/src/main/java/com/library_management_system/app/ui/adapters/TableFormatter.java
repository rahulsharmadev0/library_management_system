...
/**
 * A generic table formatter for console output that eliminates table creation
 * duplication.
 * Inspired by the ConsoleTable library for flexible and reusable table
 * formatting.
 */
public class TableFormatter<T> {
    ...
    public void display(List<T> data, Runnable onEmptyData) {
    ...
    private void renderBorder(int[] widths, int styleRow) {
        if (style.getPattern(styleRow, 0) == null)
            return;
